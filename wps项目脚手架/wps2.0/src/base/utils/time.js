 ...
 * @LastEditTime: 2019-09-21 13:34:18
 * @Description: 封装常用的的工具函数-与时间有关的一些处理函数
 */

/**
 * @description 格式化时间
 * @param {Data} data 要格式化的时间 如: new Date()
 * @returns 格式化好的时间 如 2019-03-02 12:25:25
 */
function formatCurrentDate (date) {
    ...
    strDate += ' '
    strDate += ('0' + date.getHours()).slice(-2) + ':'   // 小时
    strDate += ('0' + date.getMinutes()).slice(-2) + ':'   // 分钟
    strDate += ('0' + date.getSeconds()).slice(-2)   // 秒数
    return strDate;
}
 ...
 * @param {int} flag 返回的时间格式类型 可以不传 则返回2019-03-02  传1则返回2019年03月07日 传2则返回2019.03.02
